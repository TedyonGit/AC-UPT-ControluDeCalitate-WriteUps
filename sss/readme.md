# SSS

Shamir will be proud when you solve this.

`P1="" P2="" P3=""

We are given three large hex numbers `P1, P2, P3`.  
The hint about *Shamir* suggests this is based on secret sharing, which usually works by combining pieces with XOR.

**Steps to solve:**

1. Convert the hex strings to integers.

2. XOR them together: `X = P1 ^ P2 ^ P3`.

3. Turn the result into bytes.

4. Decode the bytes as UTF-16 big-endian.

5. The decoded string will be reversed. Reverse it to get the flag.

**Python code:**

```python
P1 = int(P1_hex, 16)
P2 = int(P2_hex, 16)
P3 = int(P3_hex, 16)

X = P1 ^ P2 ^ P3
b = X.to_bytes((max(P1.bit_length(), P2.bit_length(), P3.bit_length()) + 7)//8, 'big')
s = b.decode('utf-16-be', errors='ignore')
print(s[::-1])
```
